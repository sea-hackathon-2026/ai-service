...
from app.domain.entities.audio_chunk import AudioChunk, TTSResult
...
class MockTTSEngine(ITTSService):
    ...
    async def synthesize_stream(
        ...
        text_chunks = [
            text[i: i + _CHARS_PER_CHUNK]
            for i in range(0, len(text), _CHARS_PER_CHUNK)
        ]
        total_chunks = len(text_chunks)

        logger.info(
            "Mock TTS: job=%s, voice=%s, text_len=%d, chunks=%d",
            job_id, voice, len(text), total_chunks,
        )
