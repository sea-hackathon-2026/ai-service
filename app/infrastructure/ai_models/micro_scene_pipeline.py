...
[REFERENCE INPUTS]
Use the provided model reference image for the host's general appearance, outfit, framing, and livestream setup.
...
Keep the same reference host look, visible facial appearance, hairstyle, outfit, background, lighting, camera angle, and body framing across scenes.
...
@dataclass(frozen=True)
class MicroScenePipelineResult:
    """Result returned after rendering the full micro-scene video."""
...
def split_script_to_scenes(
    ...
    for i, chunk in enumerate(chunks, start=1):
        ...
        visual_goal = (
            f"Livestream scene for {product_name}. "
            f"{product_description or brand_style}"
        ).strip()
        ...
        scene = SceneChunk(
            ...
            image_prompt=IMAGE_PROMPT_TEMPLATE.format(
                ...
                product_action="show product clearly" if is_product_scene else "none",
                camera="medium shot"
                if scene_type.startswith("HOST")
                else "product close-up",
...
def build_veo_prompt(
    *,
    ...
    return VEO_PROMPT_TEMPLATE.format(
        ...
        product_description=product_description or "Use only the uploaded product image as factual visual reference.",
...
class MicroSceneVideoPipeline:
    ...
    def generate(
        ...
        *,
        ...
        duration_sec = get_media_duration(str(final_path), fallback=sum(s.duration_target_sec for s in scenes))

        self._notify(progress_callback, "done", 1.0, "Done")
        return MicroScenePipelineResult(
    ...
    def generate_scene_clip(
        ...
        *,
        ...
        genai_available = (
            genai is not None
            and hasattr(genai, "is_available")
            and genai.is_available()
        )
            ...
            try:
                ...
                if self._config.genai_use_imagen:
                    ...
                    if img_result:
                        ...
                        logger.info("[%s] Imagen returned nothing, using references", scene.scene_id)
        ...
        if scene.overlay_text:
            try:
                ...
                logger.exception("Failed to add overlay for %s; using clip without overlay", scene.scene_id)
